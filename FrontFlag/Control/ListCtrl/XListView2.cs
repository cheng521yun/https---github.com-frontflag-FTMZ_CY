...
{
    public class XListView2 : System.Windows.Forms.ListView
    {
        ...
        List<COLW>  _lstColW = new List<COLW>();          //记录列表初始各列的宽度。  
        ...
        private bool HandleNotify(ref Message m)
        {
            ...
            if (nmhdr.code == HDN_ITEMCHANGINGA || nmhdr.code == HDN_ITEMCHANGINGW)
            {
               ...
               {
                    //API.HDITEM hditem = (API.HDITEM)Marshal.PtrToStructure(nmheader.pHDITEM, typeof(API.HDITEM));

                   //_lstColHide 记录所有隐藏列的列序号
                    if (_lstColHide.Contains((int)nmheader.iItem))
                    {
                        Columns[(int)nmheader.iItem].Width = 0;

                        m.Result = (IntPtr)1; //修改消息值，让拖动列宽的动作失效。
                        isMsgHandled = true;
                    }
                }
            }
        ...
        public void SaveInitColWidth()
        {
            ...
            {
                COLW data = new COLW();
                data.strColName = hd.Text;
                data.nColNo = No ++;
                data.nWidth = hd.Width;

                _lstColW.Add( data );
            }
        }

        private int GetInitColW(string strColName)
        {
            foreach ( COLW data in _lstColW )
            {
                if (data.strColName == strColName)
                    return data.nWidth;
            }
        ...
        private int GetInitColW( int nColNo )
        {
            foreach ( COLW data in _lstColW )
            {
                if ( data.nColNo == nColNo )
                    return data.nWidth;
            }
        ...
        public void RestoreColWidth()
        {
            ...
            {
                strCol = hd.Text;
                int nInitColW = GetInitColW( hd.Name );
