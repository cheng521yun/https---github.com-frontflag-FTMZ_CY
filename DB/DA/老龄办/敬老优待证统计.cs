...
namespace DB.DA.老龄办
{
    partial class 敬老优待证统计
    {
        public DataTable GetBlank()
        {
            ...
            DataTable dt = Sql.GetBlankDt( DB.Tab.老龄办.敬老优待证统计.TAB );
            dt.Rows.Clear();
            Sql.Close();
            return dt;
        }
        ...
        public DataTable GetWhere( string strWhere )
        {
            ...
            string strSql = String.Format( "select * from {0} {1} order by ID desc", DB.Tab.老龄办.敬老优待证统计.TAB, strWhere );
        ...
        public DataTable GetPage( int nPageNo, string strWhere )
        {
            ...
            DataTable dt = new DataTable();
            string strSql = SQL.GetPageSql2005( DB.Tab.老龄办.敬老优待证统计.TAB, nPageNo, CONST.PageSize, DB.Tab.老龄办.敬老优待证统计.ID, strWhere, false );
            dt = Sql.ExecDataTable( strSql );
        ...
        public int GetPageMax( string strWhere )
        {
            ...
            int nPageNum = Sql.GetPageMaxPage( DB.Tab.老龄办.敬老优待证统计.TAB, strWhere, ref nRecCount, CONST.PageSize );
        ...
        public void Delete_Table()
        {
            ...
            string strSql = String.Format( "delete from {0} ", DB.Tab.老龄办.敬老优待证统计.TAB );
            Sql.Exec( strSql );
        ...
        public bool Delete_Where( string strWhere )
        {
            ...
            string strSql = String.Format( "delete from {0} where {1}", DB.Tab.老龄办.敬老优待证统计.TAB, strWhere );
            Sql.Exec( strSql );
        ...
        {
            string strWhere = String.Format( "{0}='{1}'", DB.Tab.老龄办.敬老优待证统计.ID, strID );
        ...
        public void Update_ByID( string strID, string strFld, string strVal )
        {
            ...
            string strSql = String.Format( "update {0} set {1}='{2}' where ID='{3}'", DB.Tab.老龄办.敬老优待证统计.TAB, strFld, strVal, strID );
            Sql.Exec( strSql );
        ...
        public DataTable GetCount( string strWhere )
        {
            ...
            string strSql = String.Format( "select Count(*) from {0} {1} ", DB.Tab.老龄办.敬老优待证统计.TAB, strWhere );
            dt = Sql.ExecDataTable( strSql );
