...
namespace DB.Stru.优抚科.优抚对象.三红
{
    public class 基本信息
    {
        ...
        public bool Dr2Stru( DataRow dr )
        {
            ...
            ID = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.ID ].ToString().Trim();
            CreateDate = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.CreateDate ].ToString().Trim();
            Creator = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.Creator ].ToString().Trim();
            优抚对象 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.优抚对象 ].ToString().Trim();
            优抚对象类别 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.优抚对象类别 ].ToString().Trim();
            行政区划 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.行政区划 ].ToString().Trim();
            姓名 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.姓名 ].ToString().Trim();
            身份证号码 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.身份证号码 ].ToString().Trim();
            出生日期 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.出生日期 ].ToString().Trim();
            性别 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.性别 ].ToString().Trim();
            民族 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.民族 ].ToString().Trim();
            申请类别 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.申请类别 ].ToString().Trim();
            婚姻状况 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.婚姻状况 ].ToString().Trim();
            入伍时间 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.入伍时间 ].ToString().Trim();
            退伍时间 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.退伍时间 ].ToString().Trim();
            劳动能力 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.劳动能力 ].ToString().Trim();
            生活能力 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.生活能力 ].ToString().Trim();
            就业情况 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.就业情况 ].ToString().Trim();
            户口类别 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.户口类别 ].ToString().Trim();
            户口簿住址类别 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.户口簿住址类别 ].ToString().Trim();
            联系电话 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.联系电话 ].ToString().Trim();
            工作单位 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.工作单位 ].ToString().Trim();
            户口簿住址 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.户口簿住址 ].ToString().Trim();
            实际居住地址 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.实际居住地址 ].ToString().Trim();
            优抚对象状态 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.优抚对象状态 ].ToString().Trim();
            开始待遇年度 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.开始待遇年度 ].ToString().Trim();
            停止待遇年度 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.停止待遇年度 ].ToString().Trim();
            录入时间 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.录入时间 ].ToString().Trim();
            录入人 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.录入人 ].ToString().Trim();
            负责人 = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.负责人 ].ToString().Trim();
            DelFlag = dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.DelFlag ].ToString().Trim();

            return true;
        }

        public static List<DB.Stru.优抚科.优抚对象.三红.基本信息> Dt2List( ref DataTable dt )
        {
            List<DB.Stru.优抚科.优抚对象.三红.基本信息> lst = new List<DB.Stru.优抚科.优抚对象.三红.基本信息>();
            ...
            {
                DB.Stru.优抚科.优抚对象.三红.基本信息 stru = new DB.Stru.优抚科.优抚对象.三红.基本信息();
                stru.Dr2Stru( dr );
                lst.Add( stru );
            }
        ...
        public static DataTable List2Dt( ref List<DB.Stru.优抚科.优抚对象.三红.基本信息> lst )
        {
            DB.Orm.优抚科.优抚对象.三红.基本信息 orm = new DB.Orm.优抚科.优抚对象.三红.基本信息();
            DataTable dt = orm.GetBlank();

            foreach ( DB.Stru.优抚科.优抚对象.三红.基本信息 o in lst )
            {
        ...
        public static DB.Stru.优抚科.优抚对象.三红.基本信息 StruFromList_ByID( ref List<DB.Stru.优抚科.优抚对象.三红.基本信息> list, string strID )
        {
            if ( strID.Trim() == String.Empty )
                return null;

            foreach ( DB.Stru.优抚科.优抚对象.三红.基本信息 stru in list )
            {
                if ( stru.ID == strID )
                    return stru;
            }
        ...
        public void Stru2Dr( ref DataRow dr )
        {
            if ( ID.Trim() != String.Empty )
                dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.ID ] = ID;

            CheckData();

            if ( CreateDate != String.Empty )
                dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.CreateDate ] = CreateDate;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.Creator ] = Creator;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.优抚对象 ] = 优抚对象;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.优抚对象类别 ] = 优抚对象类别;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.行政区划 ] = 行政区划;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.姓名 ] = 姓名;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.身份证号码 ] = 身份证号码;
            if ( 出生日期 != String.Empty )
                dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.出生日期 ] = 出生日期;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.性别 ] = 性别;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.民族 ] = 民族;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.申请类别 ] = 申请类别;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.婚姻状况 ] = 婚姻状况;
            if ( 入伍时间 != String.Empty )
                dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.入伍时间 ] = 入伍时间;
            if ( 退伍时间 != String.Empty )
                dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.退伍时间 ] = 退伍时间;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.劳动能力 ] = 劳动能力;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.生活能力 ] = 生活能力;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.就业情况 ] = 就业情况;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.户口类别 ] = 户口类别;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.户口簿住址类别 ] = 户口簿住址类别;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.联系电话 ] = 联系电话;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.工作单位 ] = 工作单位;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.户口簿住址 ] = 户口簿住址;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.实际居住地址 ] = 实际居住地址;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.优抚对象状态 ] = 优抚对象状态;
            if ( 开始待遇年度 != String.Empty )
                dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.开始待遇年度 ] = 开始待遇年度;
            if ( 停止待遇年度 != String.Empty )
                dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.停止待遇年度 ] = 停止待遇年度;
            if ( 录入时间 != String.Empty )
                dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.录入时间 ] = 录入时间;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.录入人 ] = 录入人;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.负责人 ] = 负责人;
            dr[ DB.Tab.优抚科.优抚对象.三红.基本信息.DelFlag ] = DelFlag;
        }

        public void CopyFrom( DB.Stru.优抚科.优抚对象.三红.基本信息 struFrom )
        {
            ID = struFrom.ID;
        ...
        /// </summary>
        /// <returns></returns>
        public string GetLogStr( DB.Stru.优抚科.优抚对象.三红.基本信息 struOrg )
        {
